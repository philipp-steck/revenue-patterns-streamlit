...
def plot_2(df_aggregate_payments, days_list):
    ...
    df_plot2 = pd.DataFrame(list(zip(days_list, percentage_payment)), columns=["days", "percentage"])
    ...
    fig = px.line(
        ...
        title=None,
        labels={"percentage": y_label},
    )

    fig.update_xaxes(
        range=[0, df_plot2["days"].max()], showspikes=True, spikedash="dot", spikethickness=2, spikemode="toaxis"
    )  # spikesnap="data")
    fig.update_yaxes(
        showspikes=True,
        spikedash="dot",
